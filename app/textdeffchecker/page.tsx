...
export default function TextDiff() {
  ...
  const [summary, setSummary] = useState<Summary>({ additions: 0, deletions: 0 });
  ...
  const compareTexts = () => {
    ...
    const result: DiffPart[] = diff.map((part, index) => {
      if (part.added) {
    ...
    if (diffResult.length === 0) {
      return (
        <p className="text-gray-500 dark:text-gray-400">
          No comparison yet. Click "Compare" to see differences.
        </p>
      );
    }
            ...
            {diffResult.map(
              (part, idx) =>
                part.type !== "added" && (
                  ...
                  >
                    <span className="w-8 text-right pr-2 text-gray-500 dark:text-gray-400">
                      {part.line}
                    </span>
                    <span
                      className={`flex-1 ${
                    ...
                    >
                      {part.type === "removed"
                        ? part.value.split(" ").map((word, i) => (
                            <span
                              key={i}
                              className="bg-red-300 dark:bg-red-700/50 px-1 mx-0.5 rounded"
                            >
                              {word}{" "}
                            </span>
                          ))
                ...
                part.type !== "removed" && (
                  ...
                  >
                    ...
                    <span
                      className={`flex-1 ${
                        part.type === "added"
                          ? "text-green-600 dark:text-green-400 bg-green-200/50 dark:bg-green-800/30"
                          : ""
                      } tooltip`}
                    ...
                    >
                      {part.type === "added"
                        ? part.value.split(" ").map((word, i) => (
                            <span
                              key={i}
                              className="bg-green-300 dark:bg-green-700/50 px-1 mx-0.5 rounded"
                            >
                              {word}{" "}
                            </span>
                          ))
    ...
    <div className="container mx-auto p-6">
      ...
      <div className="mb-6">
        ...
        <div className="flex gap-4 mb-4">
          <span className="text-sm text-gray-600 dark:text-gray-400">
            <span className="inline-block w-4 h-4 bg-green-300 dark:bg-green-700/50 mr-2"></span>
            Added
          </span>
          <span className="text-sm text-gray-600 dark:text-gray-400">
            <span className="inline-block w-4 h-4 bg-red-300 dark:bg-red-700/50 mr-2"></span>
            Removed
          </span>
        </div>
        {renderDiff()}
      ...
      <div>
        ...
        <p className="text-gray-700 dark:text-gray-300">
          ...
          {diffMode === "word" ? "characters" : "lines"} | Deletions: {summary.deletions}{" "}
          {diffMode === "word" ? "characters" : "lines"}
        </p>
      </div>
    </div>
  );
