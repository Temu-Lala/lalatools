...
export default function SecureNotes() {
  ...
  const handleUploadPDF = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Improved pattern matching for encrypted content
      const encryptedMatch = extractedText.match(/Encrypted Content:([\s\S]*?)(Generated by LALA ToolKit|$)/i);
      if (!encryptedMatch) {
        setError('No encrypted content found in the PDF. Make sure it was created by this app.');
        return;
      }
      
      let encryptedContent = encryptedMatch[1].trim();
      // Clean up the content - remove any remaining PDF artifacts
      encryptedContent = encryptedContent.replace(/[^\w+\/=]/g, '');
