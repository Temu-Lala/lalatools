...
const gregorianToEthiopian = (date: Date): { year: number; month: number; day: number } => {
  ...
  const diffDays = Math.floor((gregEpoch.getTime() - ethEpoch.getTime()) / (1000 * 60 * 60 * 24));
...
const ethiopianToGregorian = (ethYear: number, ethMonth: number, ethDay: number): Date => {
...
export default function CalendarConverter() {
  ...
  const [conversionDirection, setConversionDirection] = useState<'greg-to-eth' | 'eth-to-greg'>('greg-to-eth');
  const [gregorianDate, setGregorianDate] = useState<Date>(new Date());
  const [ethiopianDate, setEthiopianDate] = useState(gregorianToEthiopian(new Date()));
  ...
  useEffect(() => {
    if (conversionDirection === 'eth-to-greg') {
      try {
        const newDate = ethiopianToGregorian(inputDate.year, inputDate.month, inputDate.day);
        setGregorianDate(newDate);
        setEthiopianDate(gregorianToEthiopian(newDate));
      } catch (error) {
  ...
  const handleInputChange = (field: 'year' | 'month' | 'day', value: number) => {
    setInputDate(prev => ({
      ...prev,
      [field]: value,
    }));
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="flex justify-center mb-8">
            ...
            >
              <span className="mr-2">
                {conversionDirection === 'greg-to-eth' ? 'Gregorian to Ethiopian' : 'Ethiopian to Gregorian'}
              </span>
            ...
            <div className="p-6 md:p-8">
              ...
              <div className="mb-8">
                <h2 className="text-xl font-semibold mb-4">
                  {conversionDirection === 'greg-to-eth' ? 'Gregorian Date' : 'Ethiopian Date'}
                </h2>
                  ...
                  <div className="relative">
                    ...
                    <AnimatePresence>
                      {showMonthDropdown && (
                        ...
                        >
                          {(conversionDirection === 'greg-to-eth' ? GREGORIAN_MONTHS : ETHIOPIAN_MONTHS).map((month, index) => (
                            <button
                              key={month}
                              onClick={() => {
                              ...
                              className={`w-full px-4 py-2 text-left flex items-center justify-between ${inputDate.month === index ? 'bg-blue-100 dark:bg-blue-900' : 'hover:bg-gray-100 dark:hover:bg-gray-600'}`}
                            >
                              <span>{month}</span>
                              {inputDate.month === index && <Check className="h-4 w-4" />}
                            </button>
                          ))}
                        </motion.div>
                      )}
                      ...
                      max={
                        conversionDirection === 'greg-to-eth'
                          ? inputDate.month === 1 // February
                            ? isGregorianLeapYear(inputDate.year) ? 29 : 28
                            : [3, 5, 8, 10].includes(inputDate.month) ? 30 : 31
                          : inputDate.month === 12 // Pagume
                            ? isEthiopianLeapYear(inputDate.year) ? 6 : 5
                            : 30
                      }
                    />
                  </div>
              ...
              <div className="bg-blue-50 dark:bg-gray-700 rounded-lg p-6">
                <h2 className="text-xl font-semibold mb-4">
                  {conversionDirection === 'greg-to-eth' ? 'Ethiopian Date' : 'Gregorian Date'}
                </h2>
                    ...
                    {conversionDirection === 'greg-to-eth' ? (
                      <>
                        <div className="text-2xl font-bold">
                          {ethiopianDate.day} {ETHIOPIAN_MONTHS[ethiopianDate.month]} {ethiopianDate.year}
                        </div>
          ...
          >
            ...
            <div className="space-y-4 text-gray-700 dark:text-gray-300">
              <p>
                The <span className="font-medium">Ethiopian calendar</span> is the principal calendar used in Ethiopia and is 
                approximately 7-8 years behind the Gregorian calendar. It has 13 months - 12 months of 30 days 
                each and a 13th month of 5 or 6 days.
              </p>
              <p>
                The <span className="font-medium">Gregorian calendar</span> is the calendar used in most of the world. 
                It was introduced in October 1582 by Pope Gregory XIII as a modification of the Julian calendar.
              </p>
              <p>
                New Year in the Ethiopian calendar is celebrated on <span className="font-medium">September 11</span> 
